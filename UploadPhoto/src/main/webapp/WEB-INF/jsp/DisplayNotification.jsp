...
<h1>Notifications</h1>
	...
	<c:forEach items="${notify}" var="notify1">
				<tr>
				<c:choose>
					<c:when test="${not empty notify1.commentId}">
						<td>${notify1.commentId.user.name} has commented on a post!</td><td>${notify1.commentId.timestamp}</td>
					</c:when>
					<c:otherwise>
						<td>${notify1.post.user.name} has created a post!</td><td>${notify1.post.timestamp}</td>
					</c:otherwise> 
				</c:choose>
				</tr>
			</c:forEach>
