...
<script>
		...
		<div class="row">
			<table class="table table-bordered">
				...
				<c:forEach items="${friends}" var="friend">
      			  <tr>
      			       <td><img src="${friend.profilephoto}" height="50" width="50" onclick="exportuid('${friend.userId}');"/></td>
            		   <td>${friend.userId}</td>
            		   <td onclick="exportuid('${friend.userId}');">${friend.name}</td>
           			   <td>${friend.email}</td>
                  </tr>
    			</c:forEach>
  ...
  <form id="friendForm" method="POST" action="viewFriendProfile">
			 <input type="hidden"
			name="friendID"/>
